...
SFC = [[SOURCE_STREAMING_VNF, FACE_DETECTION_VNF, TRANSCODER_VNF, BACKGROUND_BLUR_VNF]]
NFV_TRANSCODER_RESOUTION = R_720P
...
def delete_sfc(sfc, sfc_id: str):
    for vnf in sfc:
        message = KubernetesAPI.delete_namespaced_deployment(vnf.service_name, sfc_id)
        print(message)
        message = KubernetesAPI.delete_namespaced_service(vnf.service_name, sfc_id)
        print(message)
